...
//{
//    public partial class ExportHistoryForm : BaseForm
//    {
...
//        private DataTable headerData; // Lưu dữ liệu phiếu xuất kho
//        private DataTable detailData; // Lưu dữ liệu chi tiết phiếu xuất kho

//        public ExportHistoryForm(int roleId) : base(roleId)
//        {
//            InitializeComponent();
...
//            if (!IsValidRole())
//            {
//                MessageBox.Show("Role không hợp lệ! Bạn không có quyền truy cập Lịch sử xuất kho.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
//                this.Close();
//                return;
//            }
...
//        {
//            // Cho phép cả nhân viên kho (roleId = 2) và quản lý (giả sử roleId = 1) truy cập
...
//            cbExporter.SelectedIndexChanged += Filter_ValueChanged;
//            cbCustomer.SelectedIndexChanged += Filter_ValueChanged;
...
//            cbExporter.SelectedIndex = 0; // Chọn "Tất cả"
//            cbCustomer.SelectedIndex = 0;
//            cbWarehouse.SelectedIndex = 0;
//            txtExitCode.Text = string.Empty;
//            LoadExportHistory();
//        }
...
//            {
//                int stockExitId = Convert.ToInt32(dgvHeaders.SelectedRows[0].Cells["colHeaderId"].Value);
//                LoadExportDetails(stockExitId);
//            }
//            else
//            {
...
//                cbCustomer.Items.Add(new Product { Id = 0, TenSanPham = "Tất cả" }); // Thêm tùy chọn "Tất cả"
...
//                    });
//                }
//                cbCustomer.SelectedIndex = 0; // Mặc định chọn "Tất cả"
//            }
//            catch (Exception ex)
//            {
...
//                cbExporter.Items.Add(new Product { Id = 0, TenSanPham = "Tất cả" });
...
//            {
//                string query = @"
//                    SELECT seh.id, seh.exit_code, seh.exit_date, seh.status, seh.note,
//                           c.name AS customer_name, w.name AS warehouse_name, u.full_name AS user_name
//                    FROM stock_exit_headers seh
//                    JOIN customers c ON seh.customer_id = c.id
...
//            catch (Exception ex)
//            {
//                MessageBox.Show("Lỗi khi tải lịch sử xuất kho: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
//            }
//        }
...
//            {
//                MessageBox.Show("Không có phiếu xuất kho nào để hiển thị!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
//                return;
//            }
...
//            Product selectedExporter = cbExporter.SelectedItem as Product;
//            Product selectedCustomer = cbCustomer.SelectedItem as Product;
//            Product selectedWarehouse = cbWarehouse.SelectedItem as Product;
//            string searchExitCode = txtExitCode.Text.Trim().ToLower();

//            foreach (DataRow row in headerData.Rows)
//            {
//                DateTime exitDate = Convert.ToDateTime(row["exit_date"]).Date;
//                string exitCode = row["exit_code"].ToString().ToLower();
//                string customerName = row["customer_name"].ToString();
//                string userName = row["user_name"].ToString();
//                string warehouseName = row["warehouse_name"].ToString();

//                bool matchesDate = exitDate >= fromDate && exitDate <= toDate;
//                bool matchesExporter = selectedExporter.Id == 0 || userName == selectedExporter.TenSanPham;
//                bool matchesCustomer = selectedCustomer.Id == 0 || customerName == selectedCustomer.TenSanPham;
//                bool matchesWarehouse = selectedWarehouse.Id == 0 || warehouseName == selectedWarehouse.TenSanPham;
//                bool matchesExitCode = string.IsNullOrEmpty(searchExitCode) || exitCode.Contains(searchExitCode);

//                if (matchesDate && matchesExporter && matchesCustomer && matchesWarehouse && matchesExitCode)
//                {
//                    dgvHeaders.Rows.Add(
//                        row["id"],
//                        row["exit_code"],
//                        Convert.ToDateTime(row["exit_date"]).ToString("dd/MM/yyyy"),
//                        row["customer_name"],
//                        row["warehouse_name"],
//                        row["user_name"],
//                        row["status"],
...
//            {
//                MessageBox.Show("Không tìm thấy phiếu xuất kho phù hợp với tiêu chí lọc!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
//            }
//        }

//        private void LoadExportDetails(int stockExitId)
//        {
//            try
//            {
//                string query = @"
...
//                    WHERE sed.stock_exit_id = @stockExitId";
//                Dictionary<string, object> paramsDict = new Dictionary<string, object> { { "@stockExitId", stockExitId } };
...
//            catch (Exception ex)
//            {
//                MessageBox.Show("Lỗi khi tải chi tiết phiếu xuất kho: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
//            }
//        }
...
//            tblFilter.Controls.Add(lblExporter, 4, 0);
//            tblFilter.Controls.Add(cbExporter, 5, 0);
//            tblFilter.Controls.Add(lblCustomer, 6, 0);
//            tblFilter.Controls.Add(cbCustomer, 7, 0);
...
//            lblExitCode = new Label { Text = "Mã phiếu:", Anchor = AnchorStyles.Left | AnchorStyles.Right, TextAlign = ContentAlignment.MiddleLeft, Font = new Font("Segoe UI", 10F) };
//            txtExitCode = new TextBox { Dock = DockStyle.Fill, Margin = new Padding(0, 5, 10, 5), Font = new Font("Segoe UI", 10F) };
...
//            tblFilter.Controls.Add(lblExitCode, 2, 1);
//            tblFilter.Controls.Add(txtExitCode, 3, 1);
//            tblFilter.Controls.Add(btnSearch, 4, 1);
//            tblFilter.SetColumnSpan(btnSearch, 2);
...
//            dgvHeaders.Columns.Add("colExitCode", "Mã phiếu");
//            dgvHeaders.Columns["colExitCode"].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
//            dgvHeaders.Columns["colExitCode"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;

//            dgvHeaders.Columns.Add("colExitDate", "Ngày xuất");
//            dgvHeaders.Columns["colExitDate"].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
//            dgvHeaders.Columns["colExitDate"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;

//            dgvHeaders.Columns.Add("colCustomerName", "Khách hàng");
//            dgvHeaders.Columns["colCustomerName"].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
...
//            dgvHeaders.Columns.Add("colUserName", "Người xuất");
...
//            dgvDetails.Columns.Add("colDetailUnitPrice", "Giá xuất (VND)");
...
//        public ImportHistoryForm(int roleId) : base(roleId)
//        {
...
//            cbWarehouse.SelectedIndexChanged += Filter_ValueChanged;
//            dgvHeaders.SelectionChanged += DgvHeaders_SelectionChanged;
//            dgvDetails.CellEndEdit += DgvDetails_CellEndEdit; // Thêm sự kiện chỉnh sửa ô
//        }
...
//            if (RoleId != 1)
//            {
//                MessageBox.Show("Bạn không có quyền chỉnh sửa phiếu nhập kho!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Warning);
//                return;
//            }
...
//                DataRow headerRow = headerData.Select($"id = {stockEntryId}")[0];

//                using (ImportWareHouseForm editForm = new ImportWareHouseForm(RoleId))
//                {
//                    // Điền dữ liệu vào form chỉnh sửa
//                    editForm.txtImportCode.Text = headerRow["entry_code"].ToString();
//                    editForm.dtpImportDate.Value = Convert.ToDateTime(headerRow["entry_date"]);
//                    editForm.cbSupplier.Text = headerRow["supplier_name"].ToString();
//                    editForm.cbImporter.Text = headerRow["user_name"].ToString();
//                    editForm.cbWarehouse.Text = headerRow["warehouse_name"].ToString();
//                    editForm.txtStatus.Text = headerRow["status"].ToString();
//                    editForm.txtNote.Text = headerRow["note"].ToString();

//                    // Load chi tiết phiếu nhập kho
//                    string query = @"
//                        SELECT sed.product_id, sed.quantity, sed.unit_price, p.name AS product_name
...
//                    DataTable details = DatabaseHelper.ExecuteQuery(query, paramsDict);

//                    foreach (DataRow row in details.Rows)
//                    {
//                        editForm.danhSachTam.Add(new Product
//                        {
//                            Id = Convert.ToInt32(row["product_id"]),
//                            TenSanPham = row["product_name"].ToString(),
//                            GiaNhap = Convert.ToDecimal(row["unit_price"]),
//                            SoLuong = Convert.ToInt32(row["quantity"])
//                        });
//                    }
//                    editForm.UpdateDataGridView();
...
//        {
//            if (RoleId != 1) return; // Chỉ admin mới được chỉnh sửa

//            int rowIndex = e.RowIndex;
//            int stockEntryId = Convert.ToInt32(dgvHeaders.SelectedRows[0].Cells["colHeaderId"].Value);
//            int productId = Convert.ToInt32(dgvDetails.Rows[rowIndex].Cells["colDetailProductId"].Value);
...
//                FlatStyle = FlatStyle.Flat,
//                Font = new Font("Segoe UI", 10F),
//                Location = new Point(headerPanel.Width - 330, 15),
...
//                FlatStyle = FlatStyle.Flat,
//                Font = new Font("Segoe UI", 10F),
//                Location = new Point(headerPanel.Width - 220, 15) // Điều chỉnh vị trí
//            };
...
//            headerPanel.Resize += (sender, e) =>
//            {
//                btnEdit.Location = new Point(headerPanel.Width - 330, 15);
//                btnDelete.Location = new Point(headerPanel.Width - 220, 15);
//                btnRefresh.Location = new Point(headerPanel.Width - 110, 15);
//                btnBack.Location = new Point(headerPanel.Width - 220, 15);
//            };
//        }
...
//        private void BtnBack_Click(object sender, EventArgs e)
//        {
//            this.Close(); // Đóng form ExportHistoryForm, quay lại ExportWareHouseForm
//        }
...
//            if (RoleId != 1)
//            {
//                MessageBox.Show("Bạn không có quyền chỉnh sửa phiếu xuất kho!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Warning);
//                return;
//            }
...
//            {
//                int stockExitId = Convert.ToInt32(dgvHeaders.SelectedRows[0].Cells["colHeaderId"].Value);
//                DataRow headerRow = headerData.Select($"id = {stockExitId}")[0];

//                using (ExportWareHouseForm editForm = new ExportWareHouseForm(RoleId))
//                {
//                    editForm.txtExportCode.Text = headerRow["exit_code"].ToString();
//                    editForm.dtpExportDate.Value = Convert.ToDateTime(headerRow["exit_date"]);
//                    editForm.cbCustomer.Text = headerRow["customer_name"].ToString();
//                    editForm.cbExporter.Text = headerRow["user_name"].ToString();
//                    editForm.cbWarehouse.Text = headerRow["warehouse_name"].ToString();
//                    editForm.txtStatus.Text = headerRow["status"].ToString();
//                    editForm.txtNote.Text = headerRow["note"].ToString();
...
//                    foreach (DataRow row in details.Rows)
//                    {
//                        editForm.danhSachTam.Add(new ExportProductWareHouse
//                        {
//                            Id = Convert.ToInt32(row["product_id"]),
//                            TenSanPham = row["product_name"].ToString(),
//                            GiaXuat = Convert.ToDecimal(row["unit_price"]),
//                            SoLuong = Convert.ToInt32(row["quantity"])
//                        });
//                    }
//                    editForm.UpdateDataGridView();
...
//            if (RoleId != 1)
//            {
//                MessageBox.Show("Bạn không có quyền xóa phiếu xuất kho!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Warning);
//                return;
//            }

//            if (dgvHeaders.SelectedRows.Count > 0)
//            {
//                DialogResult result = MessageBox.Show("Bạn có chắc chắn muốn xóa phiếu xuất kho này? Thao tác này sẽ xóa cả phiếu và các sản phẩm liên quan.", "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
//                if (result == DialogResult.No)
//                    return;
...
//                {
//                    // Xóa chi tiết phiếu trước
//                    string deleteDetailsQuery = "DELETE FROM stock_exit_details WHERE stock_exit_id = @stockExitId";
//                    Dictionary<string, object> deleteDetailsParams = new Dictionary<string, object> { { "@stockExitId", stockExitId } };
//                    DatabaseHelper.ExecuteNonQuery(deleteDetailsQuery, deleteDetailsParams);

//                    // Xóa phiếu xuất kho
//                    string deleteHeaderQuery = "DELETE FROM stock_exit_headers WHERE id = @stockExitId";
//                    Dictionary<string, object> deleteHeaderParams = new Dictionary<string, object> { { "@stockExitId", stockExitId } };
//                    DatabaseHelper.ExecuteNonQuery(deleteHeaderQuery, deleteHeaderParams);
...
//                catch (Exception ex)
//                {
//                    MessageBox.Show("Lỗi khi xóa phiếu xuất kho: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
//                }
//            }
//        }
...
//                else if (columnName == "colDetailUnitPrice" || columnName == "colDetailQuantity")
//                {
//                    // Lấy giá trị mới của giá xuất và số lượng
//                    decimal unitPrice = Convert.ToDecimal(dgvDetails.Rows[rowIndex].Cells["colDetailUnitPrice"].Value.ToString().Replace("N0", ""));
//                    int quantity = Convert.ToInt32(dgvDetails.Rows[rowIndex].Cells["colDetailQuantity"].Value);
...
//                    // Cập nhật vào bảng stock_exit_details
//                    string updateDetailQuery = "UPDATE stock_exit_details SET unit_price = @unitPrice, quantity = @quantity WHERE stock_exit_id = @stockExitId AND product_id = @productId";
//                    Dictionary<string, object> updateDetailParams = new Dictionary<string, object>
//                    {
//                        { "@unitPrice", unitPrice },
//                        { "@quantity", quantity },
//                        { "@stockExitId", stockExitId },
//                        { "@productId", productId }
//                    };
//                    DatabaseHelper.ExecuteNonQuery(updateDetailQuery, updateDetailParams);
...
//            {
//                MessageBox.Show("Lỗi khi cập nhật: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
//                LoadExportDetails(stockExitId); // Tải lại dữ liệu nếu có lỗi
//            }
//        }
...
//            {
//                cbCustomer.Items.Clear();
//                cbCustomer.Items.Add(new Product { Id = 0, TenSanPham = "Tất cả" });
...
//                    });
//                }
//                cbCustomer.DisplayMember = "TenSanPham";
...
//                    });
//                }
//                cbExporter.DisplayMember = "TenSanPham";
//                cbExporter.ValueMember = "Id";
//                cbExporter.SelectedIndex = 0;
//            }
//            catch (Exception ex)
//            {
...
//                if (headerData == null || headerData.Rows.Count == 0)
//                {
//                    MessageBox.Show("Không có phiếu xuất kho nào trong cơ sở dữ liệu!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
//                    return;
//                }
//            }
//            catch (Exception ex)
...
//            Product selectedExporter = cbExporter.SelectedItem as Product ?? new Product { Id = 0 };
//            Product selectedCustomer = cbCustomer.SelectedItem as Product ?? new Product { Id = 0 };
//            Product selectedWarehouse = cbWarehouse.SelectedItem as Product ?? new Product { Id = 0 };
//            string searchExitCode = txtExitCode.Text.Trim().ToLower();
...
//                bool matchesDate = exitDate >= fromDate && exitDate <= toDate;
//                bool matchesExporter = selectedExporter.Id == 0 || userId == selectedExporter.Id;
//                bool matchesCustomer = selectedCustomer.Id == 0 || customerId == selectedCustomer.Id;
//                bool matchesWarehouse = selectedWarehouse.Id == 0 || warehouseId == selectedWarehouse.Id;
//                bool matchesExitCode = string.IsNullOrEmpty(searchExitCode) || exitCode.Contains(searchExitCode);
...
//                FlatStyle = FlatStyle.Flat,
//                Font = new Font("Segoe UI", 10F),
//                Location = new Point(headerPanel.Width - 220, 15) // Đặt bên cạnh nút Làm mới
//            };
//            btnBack.FlatAppearance.BorderSize = 0;
...
//            dgvHeaders.Columns["colExitCode"].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
//            dgvHeaders.Columns["colExitCode"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
//            dgvHeaders.Columns["colExitCode"].ReadOnly = true; // Không cho chỉnh sửa

//            dgvHeaders.Columns.Add("colExitDate", "Ngày xuất");
//            dgvHeaders.Columns["colExitDate"].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
//            dgvHeaders.Columns["colExitDate"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
//            dgvHeaders.Columns["colExitDate"].ReadOnly = true; // Không cho chỉnh sửa
...
namespace WareHouse.Presentation.Forms
{
    public partial class ExportHistoryForm : BaseForm
    {
        ...
        private DataTable detailData; // Lưu dữ liệu chi tiết phiếu xuất kho
        private bool isInitializing; // Biến để kiểm soát quá trình khởi tạo
        private ExportWareHouseForm parentForm; // Tham chiếu đến ExportWareHouseForm

        public ExportHistoryForm(int roleId, ExportWareHouseForm parent = null) : base(roleId)
        {
            isInitializing = true; // Bắt đầu quá trình khởi tạo
            parentForm = parent; // Lưu tham chiếu đến form cha
            InitializeComponent();
        ...
        private void BtnBack_Click(object sender, EventArgs e)
        {
            if (parentForm != null)
            {
                this.Hide(); // Ẩn form ExportHistoryForm
                parentForm.Show(); // Hiện lại form ExportWareHouseForm
            }
            else
            {
        ...
        private void DgvHeaders_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            int rowIndex = e.RowIndex;
            int stockExitId = Convert.ToInt32(dgvHeaders.Rows[rowIndex].Cells["colHeaderId"].Value);
            ...
            try
            {
                ...
                Dictionary<string, object> updateParams = new Dictionary<string, object> { { "@stockExitId", stockExitId } };

                switch (columnName)
                {
                    case "colCustomerName":
                        // Cập nhật customer_id trong stock_exit_headers
                        int customerId = GetCustomerIdByName(newValue);
                        if (customerId == -1)
                        {
                        ...
                        updateQuery = "UPDATE stock_exit_headers SET customer_id = @customerId WHERE id = @stockExitId";
                        updateParams.Add("@customerId", customerId);
                        break;

                    case "colWarehouseName":
                        // Cập nhật warehouse_id trong stock_exit_headers
                        ...
                        updateQuery = "UPDATE stock_exit_headers SET warehouse_id = @warehouseId WHERE id = @stockExitId";
                        updateParams.Add("@warehouseId", warehouseId);
                        break;

                    case "colUserName":
                        // Cập nhật user_id trong stock_exit_headers
                        ...
                        updateQuery = "UPDATE stock_exit_headers SET user_id = @userId WHERE id = @stockExitId";
                        updateParams.Add("@userId", userId);
                        break;

                    case "colStatus":
                        updateQuery = "UPDATE stock_exit_headers SET status = @status WHERE id = @stockExitId";
                        updateParams.Add("@status", newValue);
                        break;

                    case "colNote":
                        updateQuery = "UPDATE stock_exit_headers SET note = @note WHERE id = @stockExitId";
        ...
        private void DgvDetails_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                MessageBox.Show("Bạn không có quyền chỉnh sửa dữ liệu!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                LoadExportDetails(Convert.ToInt32(dgvHeaders.SelectedRows[0].Cells["colHeaderId"].Value));
                return;
            }
        ...
        private void InitializeHeaderPanel()
        {
            ...
            btnBack = new Button
            {
                ...
                FlatStyle = FlatStyle.Flat,
                Font = new Font("Segoe UI", 10F),
                Location = new Point(headerPanel.Width - 110, 15) // Đặt bên cạnh nút "Làm mới"
            };
            btnBack.FlatAppearance.BorderSize = 0;
